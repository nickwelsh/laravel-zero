...
it('scaffolds the configured frontend during generation', function (): void {
    ...
    $frontend = $directory.'/generated/frontend';
    $frontendBarrel = $directory.'/frontend/index.ts';
    $provider = $frontend.'/provider.tsx';
    ...
    try {
        ...
        expect($files->get($provider))->toStartWith('// This file is generated. Do not edit directly.')
            ...
            ->and($files->get($frontendBarrel))->toContain("export * from '../generated/frontend/provider';")
            ->and($files->get($directory.'/index.ts'))->toContain("export * from './generated/schema.generated';")
...
it('bridges array wayfinder configuration to Eloquent Zero', function (): void {
    $wayfinder = [
        'method' => 'import',
        'import_path' => '~/generated/wayfinder',
    ];

    config()->set('laravel-zero.generation.use_wayfinder', $wayfinder);

    (new LaravelZeroServiceProvider(app()))->register();

    expect(config('eloquent-zero.use_wayfinder'))->toBe($wayfinder);
});

it('regenerates the React provider and appends only missing globals', function (): void {
    $files = new Filesystem;
    $directory = sys_get_temp_dir().'/laravel-zero-'.uniqid();
    $frontend = $directory.'/zero/frontend';
    $frontendBarrel = $directory.'/zero/index.ts';
    $provider = $frontend.'/provider.tsx';
    $globals = $directory.'/globals.ts';
    ...
    try {
        ...
        expect($changed)->toBe([$provider, $frontendBarrel, $globals])
            ->and($files->get($provider))->toContain("from '@/globals'", 'useMemo<ZeroContext>', 'interface AppZeroProviderProps')
            ->and($files->get($frontendBarrel))->toContain("export * from './frontend/provider';")
...
it('scaffolds a React provider without globals when configured', function (): void {
    ...
    $frontendBarrel = $directory.'/index.ts';
    $provider = $frontend.'/provider.tsx';
    $globals = $directory.'/globals.ts';
...
it('lets configured frontend classes own generated files', function (): void {
    $files = new Filesystem;
    $directory = sys_get_temp_dir().'/laravel-zero-'.uniqid();

    try {
        config()->set('laravel-zero.frontend.framework', FakeFrontend::class);
        config()->set('laravel-zero.frontend.output_path', $directory.'/generated');
        config()->set('laravel-zero.frontend.barrel_path', $directory.'/index.ts');

        expect(app(FrontendScaffolder::class)->scaffold())->toBe([
            $directory.'/generated/client.ts',
            $directory.'/index.ts',
        ])->and($files->get($directory.'/generated/client.ts'))->toContain("client = 'fake'")
            ->and($files->get($directory.'/index.ts'))->toContain("export * from './generated/client';");
    } finally {
        $files->deleteDirectory($directory);
    }
});
